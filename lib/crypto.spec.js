...
describe('Crypto', function() {
  ...
  describe('#encrypt', function() {
    ...
    it('should correctly prefix ciphertext with password salt and iv', function* () {
      stubRandomBytes.call(this);

      const encrypted = new Buffer(yield defaultInstance.encrypt('pw', 'data'), 'base64');

      expect(encrypted.slice(0, 12).toString('utf-8')).to.eql('111111111111');
      expect(encrypted.slice(12, 24).toString('utf-8')).to.eql('222222222222');
      expect(encrypted.length).to.eql(24 + 4 + 16);
    });

    function stubRandomBytes() {
      let counter = 0;
      this.sandbox.stub(crypto, 'randomBytes', function(length, cb) {
        expect(length).to.eql(12);
        counter++;
        return cb(null, new Buffer(counter.toString().repeat(12), 'utf-8'));
      });
    }
  ...
  describe('#encryptWithKey', function() {
    ...
    it('should throw error if the key is instance of Key', function () {
      let expectedError;
      try {
    ...
    it('should return a Promise', function () {
      const key = new Key('KEY', 'AAAAAAAAAAAA');
  ...
  describe('#DecryptWithKey', function() {
    ...
    it('should return a Promise', function () {
      const key = new Key('KEY', 'A'.repeat(12));
      const ciphertext = 'A'.repeat(60);
      const encrypted = defaultInstance.decryptWithKey(key, ciphertext);
