...
describe('CachedKeyring', function() {
  ...
  describe('#getKeyForCiphertext', function() {
    ...
    it('doesn\'t mix up salts with the same string representation', function*() {
      sandbox.restore();

      const keyring = new CachedKeyring(innerKeyring, 2, 2);

      let salt1 = Buffer.from([0xf5, 0xcf, 0xe2, 0xf0, 0xef, 0x68, 0xb3, 0xbb, 0x94, 0xf6, 0xbb, 0x89]);
      let salt2 = Buffer.from([0xfe, 0x99, 0x88, 0xeb, 0xd9, 0x68, 0xad, 0x8f, 0xc4, 0xe6, 0xf9, 0xfb]);

      expect(salt1).to.not.be.eql(salt2);
      expect(salt1.toString()).to.be.eql(salt2.toString());

      const key1 = yield keyring.getKeyForCiphertext(salt1);
      const key2 = yield keyring.getKeyForCiphertext(salt2);

      expect(key1.salt).to.be.eql(salt1);
      expect(key2.salt).to.be.eql(salt2);
    });
