...
describe('CachedKeyring', function() {
  ...
  describe('#getKeyForCiphertext', function() {
    ...
    it('caches the result from the original keyring', function*() {
      ...
      expect(setSpy.calledWith(salt + 'password', {value: key, lastAccessed: 12345678})).to.be.ok;
    });
    ...
    it('updates the last access time in the cache for cached keys', function*() {
      ...
      expect(setSpy.firstCall.calledWith(salt + 'password', {value: key, lastAccessed: 1})).to.be.ok;
      expect(setSpy.secondCall.calledWith(salt + 'password', {value: key, lastAccessed: 3})).to.be.ok;
    });
    ...
    it('removes old entries from the cache if necessary', function*() {
      ...
      expect(delSpy.calledWith(salt + 'password')).to.be.ok;
    });
